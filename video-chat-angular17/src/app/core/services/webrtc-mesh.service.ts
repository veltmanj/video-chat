...
export class WebrtcMeshService {
  ...
  private localFeeds = new Map<string, LocalFeedState>();
  ...
  removeLocalFeed(feedId: string): void {
    ...
    this.peers.forEach((peerState, remoteClientId) => {
      ...
      senders.forEach((sender) => {
        if (sender.track && feed.stream.getTracks().includes(sender.track)) {
  ...
  private ensurePeer(remoteClientId: string): PeerState {
    ...
    pc.ontrack = (event) => {
      ...
      const feedId = 'remote-' + remoteClientId + '-' + event.track.id;
      const remoteStream = new MediaStream([event.track]);
      const publishRemoteFeed = () => {
