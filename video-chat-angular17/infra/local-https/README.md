...
Find your LAN IP, for example `<IP-ADDRESS>`, then run:

```bash
cd ${REPO_ROOT}/video-chat-angular17/infra/local-https
./scripts/generate-cert.sh <IP-ADDRESS>
```
...
```bash
# Angular
cd ${REPO_ROOT}/video-chat-angular17
npm start
```
