...
Provider definitions are supplied through Spring configuration such as `SPRING_APPLICATION_JSON`. Each provider points at a Vault secret path containing a JWKS document in the `jwks_json` field.
